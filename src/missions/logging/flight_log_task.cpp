...
bool FlightLogTask::init()
{
    ...
    flightTrace_.clear();
    groundStopRequested_ = false;
    storageFlushRequested_ = false;
    stopped_ = false;

    // The application prewarms persistent storage before peripheral bring-up.
    // Do not reopen it here: SD file creation/preallocation and QSPI journal
    // scanning are boot-only work and must run at most once per boot path.
    storageStarted_ = storage_.healthy() || storage_.begin();
...
bool FlightLogTask::tick(uint32_t nowMs)
{
    ...
    if (storageStarted_ && !storage_.service(nowMs))
    {
        storageStarted_ = false;
        telemetryState_.health.storageOk = false;
        LOGE(logger_, nowMs, "flight_log", "all storage backends offline");
    }

    if (groundStopRequested_)
    {
        serviceGroundStop(nowMs);
        return true;
    }
    ...
    while (flightTrace_.popTransition(transition))
    {
    ...
    while (flightTrace_.popDecision(decision))
    {
    ...
    if (flightTrace_.droppedDecisionCount() != lastDroppedDecisionTraces_)
    {
        lastDroppedDecisionTraces_ = flightTrace_.droppedDecisionCount();
...
bool FlightLogTask::enqueueFastSample(uint32_t nowMs)
{
    ...
    payload.decisionSeq = flightTrace_.latestDecision().seq;
...
void FlightLogTask::drainToStorage(uint32_t nowMs, uint16_t maxRecords)
{
    ...
    {
        const uint16_t frontLength = ramBuffer_.frontLength();
        if (frontLength == 0U || !storage_.canAppend(frontLength))
        {
            break;
        }
...
void FlightLogTask::stopAtGround(uint32_t nowMs)
{
    ...
    enqueueEvent(nura_log::EventId::GROUND_STOP,
                 ...
                 ramBuffer_.used());
    groundStopRequested_ = true;
    serviceGroundStop(nowMs);
}

void FlightLogTask::serviceGroundStop(uint32_t nowMs)
{
    if (!storageStarted_)
    {
        stopped_ = true;
    ...
    drainToStorage(nowMs, NuraConstants::Logger::kFlightLogDrainRecordsPerTick);
    if (ramBuffer_.empty() && !storageFlushRequested_)
    {
        storageFlushRequested_ = storage_.requestFlush();
        if (!storageFlushRequested_)
        {
            storageStarted_ = false;
            telemetryState_.health.storageOk = false;
            return;
    ...
    if (storageFlushRequested_ && storage_.idle())
    {
        storage_.stop();
        telemetryState_.health.storageOk = false;
        stopped_ = true;

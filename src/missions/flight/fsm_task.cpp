...
FlightStateMachineTask::FlightStateMachineTask(FlightState &flightState,
    ...
    : flightState_(flightState),
      ...
      flightTrace_(flightTrace == nullptr ? fallbackFlightTrace() : *flightTrace),
      logger_(logger),
      config_(config),
...
FlightTraceBuffer &FlightStateMachineTask::fallbackFlightTrace()
{
    static FlightTraceBuffer trace;
    return trace;
}
...
bool FlightStateMachineTask::tickBenchAutoFlow(uint32_t nowMs)
{
    ...
    case State::ARMED:
        ...
        break;
    case State::LAUNCH:
#if defined(NURA_BENCH_FSM_STOP_AT_LAUNCH)
        // This bench variant intentionally holds at LAUNCH so no automatic
        // coast/apogee/deploy path can energize recovery outputs.
        return true;
#else
...
void FlightStateMachineTask::tickArmed(uint32_t nowMs)
{
    ...
    uint16_t reason = accel.source == AccelSource::LOW_G ? DECISION_REASON_PRIMARY_SENSOR : DECISION_REASON_FALLBACK_SENSOR;
...
void FlightStateMachineTask::transitionTo(State next, uint32_t nowMs)
{
    ...
    flightTrace_.pushTransition(previous, next, nowMs);
...
void FlightStateMachineTask::recordDecision(FlightDecisionKind kind,
                                            ...
                                            uint8_t count1)
{
    FlightDecisionTrace trace = flightTrace_.latestDecision();
    ++trace.seq;
    ...
    trace.count0 = count0;
    trace.count1 = count1;
    flightTrace_.pushDecision(trace);
}

...
BarometerTask::BarometerTask(BMP390HAL &barometer, TelemetryState &telemetryState, Logger &logger, const IAppConfig &config)
...
bool BarometerTask::tick(uint32_t nowMs)
{
    ...
    Bmp390Reading sample;
    const Bmp390PollResult pollResult = barometer_.poll(sample, nowMs);
    if (pollResult == Bmp390PollResult::PENDING)
    {
        return true;
    }
    if (pollResult == Bmp390PollResult::ERROR)
    {
...
bool BarometerTask::initialize(uint32_t nowMs)
{
    ...
    const bool ok = barometer_.begin(BoardPinMap::BMP390::wire(), BoardPinMap::BMP390::i2cAddress);

    if (ok)
    {
        LOGI(logger_, nowMs, "baro", "bmp390 initialized");
    }
    else
    {
        LOGW(logger_, nowMs, "baro", "bmp390 init failed");
    }
    return ok;
}
...
void BarometerTask::publishValidSample(const Bmp390Reading &sample, float rawAltitudeM)
{
    consecutiveReadFailCount_ = 0U;
    consecutiveBadValueCount_ = 0U;

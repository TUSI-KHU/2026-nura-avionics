...
ReplayResult runReplay(const Scenario &scenario, uint32_t endMs, bool forceArmedStart = true)
{
    FakeConfig config;
    ...
    for (uint32_t nowMs = 0U; nowMs <= endMs; nowMs += kTickMs)
    {
        ...
        if (flight.state != previous)
        {
            ...
            else if (flight.state == State::ARMED)
            {
                result.armedMs = nowMs;
            }
            else if (flight.state == State::LAUNCH)
            {
    ...
    result.finalState = flight.state;
    return result;
}
...
bool checkOpenRocketInitToGround()
{
    const Scenario scenario = {
        "openrocket_init_ground",
        1.0f,
        2.55f,
        10.5f,
        502.911f,
    ...
    const ReplayResult result = runReplay(scenario, 36000U, false);

    if (result.safeMs != 0U)
    {
        return fail(scenario.name, "INIT did not transition to SAFE at first tick");
    }
    if (result.armedMs == 0U || result.armedMs > kTickMs)
    {
        return fail(scenario.name, "SAFE did not naturally transition to ARMED");
    }
    if (result.launchedBeforeRealLaunch)
    {
        return fail(scenario.name, "OpenRocket replay caused launch before simulated liftoff");
    }
    if (result.launchMs == 0U || result.coastMs == 0U || result.apogeeMs == 0U ||
        result.drogueMs == 0U || result.deployMs == 0U || result.groundMs == 0U)
    {
        return fail(scenario.name, "OpenRocket replay did not visit every flight state");
    }
    if (!(result.armedMs < result.launchMs &&
          result.launchMs < result.coastMs &&
          result.coastMs < result.apogeeMs &&
          result.apogeeMs < result.drogueMs &&
          result.drogueMs < result.deployMs &&
          result.deployMs < result.groundMs))
    {
        return fail(scenario.name, "OpenRocket replay state order was not monotonic");
    }
    if (result.trueAltitudeAtApogeeDecisionM < scenario.apogeeM - 10.0f)
    {
        return fail(scenario.name, "OpenRocket replay deployed more than 10 m before apogee");
    }
    if (result.finalState != State::GROUND)
    {
        return fail(scenario.name, "OpenRocket replay final state was not GROUND");
    }

    std::printf("INFO %-28s SAFE=%lu ARMED=%lu LAUNCH=%lu COAST=%lu APOGEE=%lu DROGUE=%lu DEPLOY=%lu GROUND=%lu\n",
                scenario.name,
                static_cast<unsigned long>(result.safeMs),
                static_cast<unsigned long>(result.armedMs),
                static_cast<unsigned long>(result.launchMs),
                static_cast<unsigned long>(result.coastMs),
                static_cast<unsigned long>(result.apogeeMs),
                static_cast<unsigned long>(result.drogueMs),
                static_cast<unsigned long>(result.deployMs),
                static_cast<unsigned long>(result.groundMs));
    return pass(scenario.name);
}
...
int main()
{
    ...
    ok = checkOpenRocketInitToGround() && ok;
